...
void creat_user( int new_fd ,struct self_data data_self)
{
	...
	while(flag != 1) {
		...
		if ( i == 0 ) {
			...
			chdir("/home/llh/code/boss/user_informations");
			memset(&data_self,0,sizeof(struct self_data));
			recv(new_fd , &data_self , sizeof(struct self_data) , 0);
		...
		for( i = 0 ; i < num_client ; i++ ) {
			if(FD_ISSET(fd[i] , &fdsr)) {
				...
				if( ret <= 0 ) {
					...
					time(&now);
					chdir("/home/llh/code/boss");
					filename = fopen("system_log" ,"a+");
					if(fds[i].id != NULL)  {
						fprintf( filename ,"%s closed!!  %s\n" ,fds[i].id,ctime(&now) );
						fclose(filename);
						close(fd[i]);
					}
					FD_CLR(fd[i],&fdsr);
					...
					if(buf.flag == 2)  {
						printf("%s send:%s to others\n",buf.from,buf.sentence);
						for( j = 0 ; j < num_client ; j++)  {
							if(strcmp(fds[j].id , buf.from) != 0 )  {
								send(fds[j].socket_fd , &buf , sizeof(struct chat ) , 0 );
							}
							if(fds[j].socket_fd == 0)  {
								break;
							}
						}
					}
...
//将新的描述符添加到数组中 判断连接数会不会超
			...
			if( num_client < MAXFDS ) {
				...
				printf("a new connetced! client[%d]%s:%d\n",num_client,inet_ntoa(client_addr.sin_addr),ntohs(client_addr.sin_port));
				time(&now);
				printf("\n%s",ctime(&now));
				chdir("/home/llh/code/boss");
				filename = fopen("system_log" ,"a+");
				if(fds[i].id != NULL )  {
					fprintf( filename ,"%s connected !! time:%s\n",fds[i].id,ctime(&now) );
					fclose(filename);
				}
				if( new_fd > maxsock ) {
					maxsock = new_fd;
